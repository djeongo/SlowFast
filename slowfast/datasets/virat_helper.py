...
def load_image_lists(cfg, is_train):
    ...
    """
    list_filenames = [
        os.path.join(cfg.VIRAT.FRAME_LIST_DIR, filename)
        for filename in (
            cfg.VIRAT.TRAIN_LISTS if is_train else cfg.VIRAT.TEST_LISTS
        )
    ]
        ...
        with PathManager.open(list_filename, "r") as f:
            ...
            for line in f:
                ...
                image_paths[data_key].append(
                    os.path.join(cfg.VIRAT.FRAME_DIR, row[3])
                )
...
def load_boxes_and_labels(cfg, mode):
    ...
    Returns:
        all_boxes (dict): a dict which maps from `video_name` and
            ...
            labels for the box.
    """
    gt_lists = cfg.VIRAT.TRAIN_GT_BOX_LISTS if mode == "train" else []
    pred_lists = (
        cfg.VIRAT.TRAIN_PREDICT_BOX_LISTS
        if mode == "train"
        else cfg.VIRAT.TEST_PREDICT_BOX_LISTS
    )
    ann_filenames = [
    ...
    boxes_sample_rate = (
        4 if mode == "val" and not cfg.VIRAT.FULL_TEST_ON_VAL else 1
    )
...
def parse_bboxes_file(
    ...
    """
    Parse VIRAT bounding boxes files.
    Args:
        ann_filenames (list of str(s)): a list of VIRAT bounding boxes annotation files.
